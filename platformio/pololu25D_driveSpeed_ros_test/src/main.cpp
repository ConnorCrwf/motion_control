...
// ROS includes
#include <../.pio/libdeps/megaatmega2560/rosserial_arduino/ros.h>
#include <../.pio/libdeps/megaatmega2560/rosserial_arduino/std_msgs/Int64MultiArray.h>
#include <../.pio/libdeps/megaatmega2560/rosserial_arduino/std_msgs/Int32MultiArray.h>
#include <../.pio/libdeps/megaatmega2560/rosserial_arduino/std_msgs/Float32MultiArray.h>
#include <../.pio/libdeps/megaatmega2560/rosserial_arduino/std_msgs/Int64.h>
...
encoderInfoStruct processEncoderTicks(encoderInfoStruct encoderInfo, int count_temp);
...
std_msgs::Int32MultiArray vel_rpm;
std_msgs::Float32MultiArray vel_rad;
...
// ros::Publisher velRPM_pub("/vel_rpm", &vel_rpm);
// ros::Publisher velRad_pub("/vel_rad", &vel_rad);
...
void setup() {
 ...
 // Note that this uses the Serial stream, so we cannot use that for anything else
  ...
  //TODO: ask Alex: what is this for Int32?
  // command_data_fdbk.layout.dim = (std_msgs::MultiArrayDimension *) malloc(sizeof(std_msgs::MultiArrayDimension) * 1);
  command_data_fdbk.layout.dim = allocateArray(std_msgs::MultiArrayDimension, 1);
  command_data_fdbk.layout.dim[0].label = "command_data";
  command_data_fdbk.layout.dim[0].size = 4;
  command_data_fdbk.layout.dim[0].stride = 1*4;
  //askAlex: what is this for Int32?
  command_data_fdbk.data = (long int *)malloc(sizeof(long int)*4);
  command_data_fdbk.layout.dim_length = 0;
  command_data_fdbk.data_length = 4;
...
void loop() {
  ...
  // encoderInfoLeft.angPos_ticks = countLeft;
  // encoderInfoRight.angPos_ticks = countRight;
  

  int32_t angPos_ticks[2] = {encoderInfoLeft.angPos_ticks, encoderInfoRight.angPos_ticks};
  ...
  for (int i = 0; i < 2; i++){
    ...
    // vel_rpm.data[i] = angVel_rpm[i];
    // vel_rad.data[i] = angVel_rad[i];
  }
  posTicks_pub.publish(&pos_ticks);
  // posRad_pub.publish(&pos_rad);
  // velRPM_pub.publish(&vel_rpm);
  // velRad_pub.publish(&vel_rad);
...
encoderInfoStruct processEncoderTicks(encoderInfoStruct encoderInfo, int count_temp)  {  
  ...
  encoderInfo.angPos_rad = (float)(encoderInfo.angPos_ticks/(48*20.4)*TWO_PI); 
